...
def ohtm_pipeline(
        working_folder: str = "", source: list = [ "", ""], source_path: str = "", stopword_file:str = "", allowed_postags_settings: list =['NOUN', 'PROPN', 'VERB', 'ADJ', 'NUM', 'ADV'], save_name: str="",load_file_name: str="", mallet_path:str="", interview_id: str = "",
        ...
        use_topic_modeling:bool = False, use_corelation:bool = False, save_top_words: bool =False, print_json:bool=False,
        ...
        search_for_topics_in_interview:bool = False, by_particle: bool = False, by_list:bool = False, pos_filter_setting: bool = False, lemma: bool=False, save_model: bool = False,
        infer_new_documents: bool = False, trained_json_name: str = "", save_separate_json:bool = False
):

    if infer_new_documents == False:
        if creat_json == True:
            print("Starting creating json")
            top_dic = dictionary_creation(source=source, source_path=source_path)
            if save_json == True:
                save_json_function(top_dic=top_dic, working_folder=working_folder, folder_name = save_name, save_name=save_name)
        ...
        if use_preprocessing == True:
            ...
            top_dic = preprocessing(top_dic = top_dic, stoplist_path = stopword_file, allowed_postags_settings=allowed_postags_settings, by_list=by_list, lemma=lemma, by_particle=by_particle, pos_filter_setting=pos_filter_setting)
        ...
        if use_topic_modeling == True:
            ...
            top_dic = topic_training_mallet(corpus_dictionary = top_dic, working_folder=working_folder, save_name = save_name, topics=topics, mallet_path=mallet_path,
                                            optimize_interval_mallet=optimize_interval_mallet, iterations_mallet=iterations_mallet, alpha = alpha, save_model = save_model,
                                            save_json=save_json)
        ...
        if use_corelation == True:
            try:
                ...
                print("------ERROR------")
                print("Not able to enrich the model, because the necessary pipeline is not installed")
    ...
    if infer_new_documents == True:
        ...
        infer_dic = dictionary_creation(source=source, source_path=source_path)
        ...
        if model_dic["settings"]["preprocessing"]["preprocessed"] == "True":
            ...
            infer_dic = preprocessing(top_dic=infer_dic, allowed_postags_settings=model_dic["settings"]["preprocessing"]["allowed_postags"],
                                        by_list=by_list, lemma=lemma, by_particle=by_particle,
                                        pos_filter_setting=pos_filter_setting, stop_words = stop_words, infer_new_documents = infer_new_documents)
        ...
        infer_dic = topic_inferring(corpus_dictionary=infer_dic, model_name = trained_json_name, mallet_path=mallet_path, working_folder=working_folder,
                                  topics = int(model_dic["settings"]["topic_modeling"]["topics"]), iterations_mallet=int(model_dic["settings"]["topic_modeling"]["iterations_mallet"]),
                                  random_seed_mallet = int(model_dic["settings"]["topic_modeling"]["random_seed_mallet"])
                                  )

        if save_separate_json == True:
            save_json_function(top_dic=infer_dic, working_folder=working_folder, folder_name = trained_json_name, save_name=trained_json_name+"_inferred")
            print("Inferred json was saved")
            top_dic = infer_dic
        else:
            top_dic = combine_infer(top_dic = model_dic, infer_dic = infer_dic)
            save_json_function(top_dic=top_dic, working_folder=working_folder, folder_name = trained_json_name, save_name=trained_json_name)
