...
def save_json_function(ohtm_file, working_folder: str = "", save_name: str = ""):
    ...
    with open(os.path.join(folder_path, save_name + ".ohtm_pipeline"), "w", encoding="utf-8") as f:
...
def load_json_function(load_file_name: str = "", working_folder: str = ""):
    with open(os.path.join(working_folder, load_file_name + ".ohtm_pipeline")) as f:
        ohtm_file = json.load(f)
        ohtm_file = convert_ohtm_file(ohtm_file)
        print(f"The ohtm_file '{load_file_name}.ohtm_pipeline' was loaded")
        return ohtm_file
